    ...
    public static class HttpHandler
    {
        ...
        private static void HandleServersRequest(IHttpResponseDelegate response)
        {
            ...
            try
            {
                ...
                outputStream += Razor.Parse(template, sessions);
            }
            catch (RazorEngine.Templating.TemplateCompilationException e)
            {
                status = "500 Internal Server Error";
                outputStream += "<pre>";
                outputStream += e.ToString();

                outputStream += "\n";
                foreach (var err in e.Errors)
                {
                    outputStream += err.ToString() + "\n";
                    outputStream += err.ErrorText + "\n";
                }

                outputStream += e.SourceCode;
            ...
            }, new BufferedProducer(outputStream));
        }
