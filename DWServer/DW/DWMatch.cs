...
namespace DWServer
{
    class DWMatch
    {
        ...
        public static void CleanSessions()
        {
            ...
            IEnumerable<KeyValuePair<ulong, MatchMakingInfoT5>> sessions = null;
            lock (_sessions)
            {
            ...
            foreach (var session in sessions)
            {
                ...
                if (_sessions[session.Key].Reachable)
                    _sessions[session.Key].LastTouched = DateTime.Now;

            }

            foreach (var session in sessions)
            {
                if ((DateTime.Now - ((MatchMakingInfoT5)session.Value).LastTouched).TotalSeconds > 120)
                {
                    _sessions.Remove(session.Key);
                    Console.WriteLine("Deleted old session: {0}", session.Key);
                }
            }
        ...
        private static void FindSessions(MessageData mdata, DWMessage packet)
        {
            ...
            if (titleID == TitleID.T5)
            {
                if (queryType == 2)
                {
                    FindZombieSessions((int)count, mdata, packet);
                }
                else if (queryType == 4)
                {
        ...
        private static void FindSessionsPaged(MessageData mdata, DWMessage packet)
        {
            try
            {
                ...
                lock (_sessions)
                {
                    ...
                    {
                        //Set some bollocks
                        session.Value.StatDDL = cd;
                        session.Value.NetcodeVersion = (int)ca;
                        session.Value.Wager = wager;
                        session.Value.Serialize(reply);
                    }
        ...
        public static Dictionary<ulong, MatchMakingInfoT5> Sessions
        {
            get { return _sessions; }
        }

        private static Dictionary<ulong, MatchMakingInfoT5> _sessions = new Dictionary<ulong, MatchMakingInfoT5>();
    }
    ...
    public class MatchMakingInfoT5 : bdMatchMakingInfo
    {
        ...
        public DateTime LastTouched { get; set; } //Last time updated
    ...
    public class bdMatchMakingInfo
    {
        ...
        public string CountryCode
        {
            get
            {
                return new WebClient().DownloadString("http://server.repziw4.de/getloc.php?ip=" + someIP);
            }
        }
